...
/**
 *
 * @returnstd::vector with the domain actions
 */
std::vector<Action*> Utils::GetActions()
{
 ...
 */
std::vector<std::vector<std::string>> Utils::IsApplicable(LiteralList* state, Action* action)
{

    std::vector<std::vector<std::string>> parameter_list;

    // Keep action parameters on a localstd::vector of pair<paramName, value>
    ...
    for (auto preconditions : *action->getPrecond()) {
        ...
        auto* new_predicate_pair = new std::pair<Predicate*, bool>;
    ...
    ------------------------------------------------------------------------------------------------------------------*/
    const int n = static_cast<int>(action_params.size()); // Insert N here: how many loops do you need?
    std::vector<int> i(n + 1); // This holds the index of each loop. It's 1 bigger than the number of loops.
    ...
    while (i[n] == 0) { // Remember, you're only using indices i[0], ..., i[n-1]. The (n+1)th index, i[n],
                ...
                if (precondition->second == 1) { // Equality true
                    ...
                    for (unsigned int z = 0; z < precondition->first->getArgs()->size(); z++) {
                        for (unsigned int k = 0; k < precondition->first->getArgs()->size(); k++) {
                            if (z != k && precondition->first->getArgs()->at(z) != precondition->first->getArgs()->at(k)) {
                                all_args_eq = false;
                            }
                        }
                    }
        ...
        for (unsigned int outer = 0; outer < precondition_list.size(); outer++)
            for (unsigned int inner = 0; inner < precondition_list.at(outer)->first->getArgs()->size(); inner++)
                precondition_list.at(outer)->first->getArgs()->at(inner) = action->getPrecond()->at(outer)->first->getArgs()->at(inner);
...
/**
 *
 * @returnstd::vector containing pairs of applicable actions and their applicable params values on the state supplied
 */
std::vector<Action*>* Utils::ApplicableActions(LiteralList* state)
{

    std::vector<Action*> actions = this->GetActions();
    std::vector<std::pair<Action*, std::vector<std::vector<std::string>>>> applicable_actions;

    for (auto& action : actions) {
        std::pair<Action*, std::vector<std::vector<std::string>>> applicable_actions_params;
        applicable_actions_params.first = action;
        applicable_actions_params.second = this->IsApplicable(state, action);
        if (!applicable_actions_params.second.empty())
            applicable_actions.push_back(applicable_actions_params);
    }
        ...
        while (state_index < state_size && !applied) {
            ...
            if (effect_predicate->getName() == state_predicate->getName()) {
                ...
                for (unsigned int arg_index = 0; arg_index < state_predicate->getArgs()->size(); arg_index++) {
                    std::string param = effect_predicate->getArgs()->at(arg_index);
...
bool Utils::LiteralsEqual(Literal* literal_1, Literal* literal_2)
{
    ...
    return literal_1->first->getName() == literal_2->first->getName() && *literal_1->first->getArgs() == *literal_2->first->getArgs() && literal_1->second == literal_2->second;
}
...
std::vector<Action*>* Utils::UnrollActions(std::vector<std::pair<Action*, std::vector<std::vector<std::string>>>>* rolled_actions)
{
    auto* unrolled_actions = new std::vector<Action*>();

    // Fore each rolled action.
    for (const std::pair<Action*, std::vector<std::vector<std::string>>>& rolled_action : *rolled_actions)
